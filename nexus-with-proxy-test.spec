...
rm -rf ./*
proxychains4 git clone %{URL} $RPM_SOURCE_DIR/nexus-public
...
proxychains4 git %{SETGITENV} fetch --tags
git %{SETGITENV} checkout -b %{BRANCH} origin/%{BRANCH} --
cd %{GITDIR} && proxychains4 %{GITDIR}/mvnw clean install %{SKIPTEST} %{BUILDVERSION}
